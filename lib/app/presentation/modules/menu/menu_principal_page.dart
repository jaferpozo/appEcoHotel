...
class MenuPrincipalPage extends GetView<MenuPrincipalController> {
  ...
  Widget build(BuildContext context) {
    return WorkAreaMenuPageWidget(
      ...
      contenido:  Obx(() => controller.status.value == ConnectionStatus.online?getContenido():getContenidoSinInternet()),
    );
  }
  getContenido(){
  ...
  getListaDatosModulos(){
    ...
    Widget listaU=ListView.builder(
      shrinkWrap: true,
      itemCount: controller.listaModulo != null ? controller.listaModulo.length : 0,
      ...
      Stack(children: [
        Column(
          ...
          Container(
            ...
            const Text('No tiene Conexión a Internet',  style: TextStyle(
              fontWeight:
              FontWeight.bold,
              fontSize: 18,
              color: Colors.black,
  ...
  Widget btnR() {
    ...
    Container(
    padding: const EdgeInsets.symmetric(vertical: 25.0,horizontal: 50),child: const Text('La Conexión a Internet esta restablecida favor Cargue los Módulos de Mi Upc',  style: TextStyle(
        fontWeight:
        FontWeight.bold,
        fontSize: 18,
        color: Colors.black,
